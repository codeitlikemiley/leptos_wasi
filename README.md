...
middleware component can wrap this terminal service in-process on Spin or be
precomposed before `wasmtime serve`; it does not require another Handler builder
method. See [WASIp3 HTTP Middleware](./MIDDLEWARE.md) for the experimental,
pinned vNext example and the boundary between component middleware,
server-function middleware, and ingress policy. The reusable implementation is
independently versioned as `wasi-http-middleware 0.1.0-alpha.1`; the fixture in
this repository is only a protocol and browser compatibility probe.
...
- To exercise the experimental pinned WASIp3 middleware fixture, install the
  ...
  `./scripts/run-middleware-tests.sh`. Set `MIDDLEWARE=1` on either browser
  command to prove middleware wrapping, unauthenticated server-action
  rejection, authenticated success, and lazy hydration without making `/pkg`
  split assets private.
